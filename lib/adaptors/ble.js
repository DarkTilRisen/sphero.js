...
//        console.log(this.noblePeripheral.id);
     ...
     if (this.noblePeripheral != null) {
         ...
         ble.on("discover", function (peripheral) {
             if (peripheral.id === self.uuid) {
                 ...
                 if (scanning == 0) {
                     ble.stopScanning();
                     to_connect_callbacks.forEach(x => x())
                     to_connect_callbacks = [];
                 }
             }
         });
         if (ble.state === "poweredOn") {
...
Adaptor.prototype._connectService = function (serviceId, callback) {
    ...
    //console.log(this._connectedServices());
